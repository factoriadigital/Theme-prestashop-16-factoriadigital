...
<div id="casual_tabs" class="container">
  <section id="content">
    ...
    <div class="filter-container">
      <section class="casual_products clearfix" id="portfolio-items"> {foreach from=$products item=product name=factoCat}
          ...
          <div class="product_isotope">
            <div class="productlisting">
              <div class="border_inside">
                <div class="prod_img">
                  <div class="productowl-carousel productowl-demo"> {assign var="prod_id" value=$product.id_product}
                    {if isset($prodImages.$prod_id)}
                    {foreach from=$prodImages.$prod_id item=image name=thumbnails}
                    {assign var=imageIds value="`$prod_id`-`$image.id_image`"} <a href="{$product.link}" title="{$product.name|escape:html:'UTF-8'}" class="prodtimage product_image hoverproimg"> <img id="productImage_{$image.id_image}_{$product.id_product}" src="{$link->getImageLink($product.link_rewrite, $imageIds, 'home_default')|escape:'html':'UTF-8'}" alt="{$product.name|escape:html:'UTF-8'}" height="{$homeSize.height}" width="{$homeSize.width}" /> </a> {/foreach}
                    {/if} </div>
                  <div class="cartbottom_btn"> {if isset($quick_view) && $quick_view}
                    ...
                    <div class="compare"> <a class="add_to_compare" href="{$product.link|escape:'html':'UTF-8'}" data-id-product="{$product.id_product|intval}" title="{l s='Add to compare' mod='blockfactocategories'}"><i class="icon-signal"></i></a> </div>
                    {/if} </div>
                </div>
