...
<div id="htmlcontent_{$hook|escape:'htmlall':'UTF-8'}" class="container">
	<ul class="htmlcontent-home clearfix marg-li">
		{foreach name=items from=$htmlitems item=hItem}
			{if $hook == 'left' || $hook == 'right' || $hook =='factoadvertisehome03'}
			...
			{/if}
					...
					{/if}
						{if $hItem.image}
							<img src="{$link->getMediaLink("`$module_dir`views/img/`$hItem.image`")}" class="item-img {if $hook == 'left' || $hook == 'right'}img-responsive{/if}" title="{$hItem.title|escape:'htmlall':'UTF-8'}" alt="{$hItem.title|escape:'htmlall':'UTF-8'}" width="{if $hItem.image_w}{$hItem.image_w|intval}{else}100%{/if}" height="{if $hItem.image_h}{$hItem.image_h|intval}{else}100%{/if}"/>
						{/if}
