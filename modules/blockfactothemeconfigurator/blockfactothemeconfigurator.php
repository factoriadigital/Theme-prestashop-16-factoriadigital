...
class BlockFactoThemeConfigurator extends Module
{
	...
	{
		$this->name = 'blockfactothemeconfigurator';
		...
		parent::__construct();
		$this->displayName = $this->l('Configurador de la plantilla');
	...
	public function createAjaxController()
	{
		...
		if (is_array($languages))
			foreach ($languages as $language)
				$tab->name[$language['id_lang']] = 'blockfactothemeconfigurator';
		$tab->class_name = 'AdminBlockFactoThemeConfigurator';
	...
	{
		if ($tab_id = (int)Tab::getIdFromClassName('AdminBlockFactoThemeConfigurator'))
		{
			$tab = new Tab($tab_id);
			$tab->delete();
		}
	...
	public function install()
	{
		...
		if (!parent::install()
			...
			!$this->registerHook('factoAdvertiseHome01') ||
			!$this->registerHook('factoAdvertiseHome02') ||
			!$this->registerHook('factoAdvertiseHome03') ||
	...
	{
		return (
			Db::getInstance()->Execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'blockfactothemeconfigurator`') &&
			Db::getInstance()->Execute('
			CREATE TABLE `'._DB_PREFIX_.'blockfactothemeconfigurator` (
	...
	protected function installFixture($hook, $id_image, $id_shop, $id_lang)
	{
		...
		$sizes = @getimagesize((dirname(__FILE__).DIRECTORY_SEPARATOR.'views/img'.DIRECTORY_SEPARATOR.'banner-img'.(int)$id_image.'.jpg'));
		$width = (isset($sizes[0]) && $sizes[0])? (int)$sizes[0] : 0;
		$height = (isset($sizes[1]) && $sizes[1])? (int)$sizes[1] : 0;

		$result &= Db::getInstance()->Execute('
			INSERT INTO `'._DB_PREFIX_.'blockfactothemeconfigurator` (
	...
	public function installFixtures($languages = null)
	{
		...
		foreach ($languages as $language)
		{
			...
			for ($i = 1; $i < 4; $i++)
				$result &= $this->installFixture('factoadvertisehome01', $i, $this->context->shop->id, $language['id_lang']);
			for ($i = 4; $i < 8; $i++)
				$result &= $this->installFixture('factoadvertisehome02', $i, $this->context->shop->id, $language['id_lang']);
			for ($i = 8; $i < 9; $i++)
				$result &= $this->installFixture('factoadvertisehome03', $i, $this->context->shop->id, $language['id_lang']);
			for ($i = 10; $i < 11; $i++)
				$result &= $this->installFixture('right', $i, $this->context->shop->id, $language['id_lang']);
		}
	...
	{
		$images = array();
		if (count(Db::getInstance()->executeS('SHOW TABLES LIKE \''._DB_PREFIX_.'blockfactothemeconfigurator\'')))
			$images = Db::getInstance()->executeS('SELECT image FROM `'._DB_PREFIX_.'blockfactothemeconfigurator`');
		foreach ($images as $image)
			$this->deleteImage($image['image']);

		if (!Db::getInstance()->Execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'blockfactothemeconfigurator`') || !$this->_removeAjaxContoller() || !parent::uninstall())
			return false;
...
//	}
	...
	public function hookdisplayHeader()
	{
		...
		else
		{
			if (Configuration::get('PS_TC_THEME') != '')
				$this->context->controller->addCss($this->_path.'views/css/'.Configuration::get('PS_TC_THEME').'.css', 'all');
		}
	...
	public function hookfactoAdvertiseHome01()
	{
		$this->context->smarty->assign(array(
			'htmlitems' => $this->getItemsFromHook('factoadvertisehome01'),
			'hook' => 'factoadvertisehome01'
		));

		return $this->display(__FILE__, 'hook.tpl');
	}
	public function hookfactoAdvertiseHome02()
	{
		$this->context->smarty->assign(array(
			'htmlitems' => $this->getItemsFromHook('factoadvertisehome02'),
			'hook' => 'factoadvertisehome02'
		));

		return $this->display(__FILE__, 'hook.tpl');
	}
	
	public function hookfactoAdvertiseHome03()
	{
		$this->context->smarty->assign(array(
			'htmlitems' => $this->getItemsFromHook('factoadvertisehome03'),
			'hook' => 'factoadvertisehome03'
		));
	...
	public function hookDisplayFooter()
	{
		...
		if ((int)Configuration::get('PS_TC_ACTIVE') == 1 && Tools::getValue('live_configurator_token') && Tools::getValue('live_configurator_token') == $this->getLiveConfiguratorToken() && Tools::getIsset('id_employee'))
		{
			...
			$ad_image = $this->_path.'views/img/'.$this->context->language->iso_code.'/advertisement.png';

			if (!file_exists($ad_image))
				$ad_image = $this->_path.'views/img/en/advertisement.png';
			...
			$this->smarty->assign(array(
				...
				'advertisement_url' => 'https://www.factoriadigital.com/'
	...
	protected function getItemsFromHook($hook)
	{
		...
		SELECT *
		FROM `'._DB_PREFIX_.'blockfactothemeconfigurator`
	...
	protected function removeItem()
	{
		...
		if ($image = Db::getInstance()->getValue('SELECT image FROM `'._DB_PREFIX_.'blockfactothemeconfigurator` WHERE id_item = '.(int)$id_item))
			$this->deleteImage($image);

		Db::getInstance()->delete(_DB_PREFIX_.'blockfactothemeconfigurator', 'id_item = '.(int)$id_item);

		if (Db::getInstance()->Affected_Rows() == 1)
		{
			Db::getInstance()->execute('
				UPDATE `'._DB_PREFIX_.'blockfactothemeconfigurator`
	...
	protected function updateItem()
	{
		...
		{
			if ($old_image = Db::getInstance()->getValue('SELECT image FROM `'._DB_PREFIX_.'blockfactothemeconfigurator` WHERE id_item = '.(int)$id_item))
		...
		if (!Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'blockfactothemeconfigurator` SET
					title = \''.pSQL($title).'\',
		...
		{
			if ($image = Db::getInstance()->getValue('SELECT image FROM `'._DB_PREFIX_.'blockfactothemeconfigurator` WHERE id_item = '.(int)Tools::getValue('item_id')))
				$this->deleteImage($image);
	...
	public function getContent()
	{
		...
		$html .= $this->renderBlockfactoThemeConfiguratorForm();
	...
	protected function renderBlockfactoThemeConfiguratorForm()
	{
		$id_shop = (int)$this->context->shop->id;
		$items = array();
		$hooks = array();
		...
		foreach ($this->languages as $language)
		{
			$hooks[$language['id_lang']] = array(
				'factoadvertisehome01',
				'factoadvertisehome02',
				'factoadvertisehome03',
				'home',
				'top',
				'left',
				'right',
			...
			foreach ($hooks[$language['id_lang']] as $hook)
				$items[$language['id_lang']][$hook] = Db::getInstance()->ExecuteS('
					SELECT * FROM `'._DB_PREFIX_.'blockfactothemeconfigurator`
		...
		$this->context->smarty->assign('htmlitems', array(
			'items' => $items,
			'theme_url' => $this->context->link->getAdminLink('AdminBlockfactoThemeConfigurator'),
			'lang' => array(
	...
	protected function getConfigurableModules()
	{
		...
		if ($this->context->shop->getBaseURL())
		{
			...
			$desc = '<a class="btn btn-default" href="'.$url.'" onclick="return !window.open($(this).attr(\'href\'));" id="live_conf_button">'
				...
				.$this->l('Please disable "custom theme color controller" before use this tool, you can use predifined configurator (OR) custom color controller');
		}
		else
		...
		return array(
			array(
				'label' => $this->l('Display Stylish Categories on home page'),
				'name' => 'blockfactocategories',
				'value' => (int)Validate::isLoadedObject($module = Module::getInstanceByName('blockfactocategories')) && $module->isEnabledForShopContext(),
				'is_module' => true,
			),
			array(
				'label' => $this->l('Simple slider on home page'),
				'name' => 'simplesliders',
				'value' => (int)Validate::isLoadedObject($module = Module::getInstanceByName('simplesliders')) && $module->isEnabledForShopContext(),
				'is_module' => true,
			),
			//array(
				//'label' => $this->l('Display links to your store\'s social accounts (Twitter, Facebook, etc.)'),
				//'name' => 'blockfactosocial',
				//'value' => (int)Validate::isLoadedObject($module = Module::getInstanceByName('blockfactosocial')) && $module->isEnabledForShopContext(),
				//'is_module' => true,
			//),
			array(
			...
			array(
				'label' => $this->l('Display Featured products on footer'),
				'name' => 'blockfeaturedfooter',
				'value' => (int)Validate::isLoadedObject($module = Module::getInstanceByName('blockfeaturedfooter')) && $module->isEnabledForShopContext(),
				'is_module' => true,
			),
			//array(
				//'label' => $this->l('Display the Newsletter block on the footer'),
				//'name' => 'blockfactonewsletter',
			//	'value' => (int)Validate::isLoadedObject($module = Module::getInstanceByName('blockfactonewsletter')) && $module->isEnabledForShopContext(),
				//'is_module' => true,
		//	),
			array(
			...
			array(
				...
				'desc' => 'Limpiar caché del navegador después de cambios.'
			),
			//array(
		...
		//	),
			...
			//),
			//array(
				//'label' => $this->l('Display predefined color Configurator'),
				//'name' => 'live_conf',
				//'value' => (int)Tools::getValue('PS_TC_ACTIVE', Configuration::get('PS_TC_ACTIVE')),
				//'hint' => $this->l('Please disable "custom theme color controller" before use this tool, This tool allows you to choose predifned color changes in your theme.'),
			//	'desc' => $desc
			//),
			
			array(
				'label' => $this->l('(OR) Display custom theme color controller'),
				'name' => 'factothemecontroller',
				'value' => (int)Validate::isLoadedObject($module = Module::getInstanceByName('factothemecontroller')) && $module->isEnabledForShopContext(),
				'is_module' => true,
				'hint' => $this->l('Please disable "Predifined color controller" before use this tool, This customization tool allows you to make custom color changes in your theme.')
			),
			array(
