...
include_once('simplesliders.php');

$home_slider = new SimpleSliders();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'simplesliders_slides` SET `position` = '.(int)$position.'
			WHERE `id_simplesliders_slides` = '.(int)$id_slide
		);
