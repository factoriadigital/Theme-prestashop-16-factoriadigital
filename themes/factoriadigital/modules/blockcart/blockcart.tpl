...
<div class="col-sm-4 clearfix{if $PS_CATALOG_MODE} header_user_catalog{/if} header-col-sm-4">
{/if}
<div class="wishlinks">
            	...
            	{if $comparator_max_item}
                    <form method="post" action="{$link->getPageLink('products-comparison')|escape:'html':'UTF-8'}" onsubmit="true" class="compare-form">
                        <button type="submit" class="compare_button bt_compare bt_compare{if isset($paginationId)}_{$paginationId}{/if}">
                            <div class="header-compare">{l s='Compare' mod='blockcart'} (<span class="total-compare-val">{count($compared_products)}</span> )</div>
                        </button>
	...
	<div class="shopping_cart">
    	<div class="desktop_vs">
            <a href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">
                <div class="cart_txt">{l s='Cart' mod='blockcart'}</div>
              <span class="totalprice"><!---Span price--->
                <span class="ajax_cart_quantity{if $cart_qties == 0} unvisible{/if} price_circle">{$cart_qties}</span>
                <span class="ajax_cart_no_product{if $cart_qties > 0} unvisible{/if} price_circle">0</span>
